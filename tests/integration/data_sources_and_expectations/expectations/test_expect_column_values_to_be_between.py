...
def test_include_unexpected_rows(batch_for_datasource: Batch) -> None:
    """Test that include_unexpected_rows works correctly for ExpectColumnValuesToBeBetween."""
    ...
    # Should contain rows where numbers column is not between 2 and 4 (1 and 5, plus None)
    assert len(unexpected_rows_df) > 0

    # Check that the unexpected rows contain the expected columns
    assert NUMERIC_COLUMN in unexpected_rows_df.columns
