...
def test_include_unexpected_rows(batch_for_datasource: Batch) -> None:
    """Test that include_unexpected_rows works correctly for ExpectColumnValuesToBeInSet."""
    ...
    # Should contain 1 row where numbers column has value 3 (not in [1,2])
    assert len(unexpected_rows_df) == 1

    # Check that the unexpected row contains the expected columns
    assert NUMBERS_COLUMN in unexpected_rows_df.columns
