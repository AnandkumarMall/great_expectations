...
from great_expectations.metrics.column.distinct_values_missing_from_set_count import (
    ColumnDistinctValuesMissingFromSetCount,
    ColumnDistinctValuesMissingFromSetCountResult,
)
...
class TestColumnDistinctValuesMissingFromSetCount:
    ...
    def test_all_values_present(self, batch_for_datasource: Batch) -> None:
        ...
        metric = ColumnDistinctValuesMissingFromSetCount(column=COLUMN_NAME, value_set=["a", "b"])
        metric_result = batch_for_datasource.compute_metrics(metric)

        assert isinstance(metric_result, ColumnDistinctValuesMissingFromSetCountResult)
    ...
    def test_some_values_missing(self, batch_for_datasource: Batch) -> None:
        ...
        metric = ColumnDistinctValuesMissingFromSetCount(
            column=COLUMN_NAME,
    ...
    def test_empty_value_set(self, batch_for_datasource: Batch) -> None:
        ...
        metric = ColumnDistinctValuesMissingFromSetCount(column=COLUMN_NAME, value_set=[])
        metric_result = batch_for_datasource.compute_metrics(metric)
